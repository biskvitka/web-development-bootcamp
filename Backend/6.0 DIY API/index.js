...
app.get("/jokes/:id", (req, res) => {
  var reqNumber = parseInt(req.params.id);
  var foundJoke = jokes.find((joke) => joke.id === reqNumber);
  res.json(foundJoke);
...
app.get("/filter", (req, res) => {
  var jokeType = req.query.type;
  var foundJokes = jokes.filter((joke) => joke.jokeType === jokeType);
  res.json(foundJokes);
});

//4. POST a new joke
app.post("/jokes", (req, res) => {
  jokes.push({id: jokes[jokes.length-1].id + 1 , 
...
app.patch("/jokes/:id", (req, res) => {
  ...
  var foundJoke = jokes.find((joke) => joke.id === id);
  ...
  if(foundJoke) {
    ...
    res
      .status(404)
      .json({ error: `Joke with id: ${id} not found. No jokes were patched.` });
  }
});
